...
class GameTicTacToeHard : AppCompatActivity() {
    ...
    private fun result(title: String) {
        val databaseReference = FirebaseDatabase.getInstance().getReference("blockBaby/TicTacToe/Hard")
        databaseReference.child("youHardTicTacToe").setValue(noughtsScore)
        databaseReference.child("computerHardTicTacToe").setValue(crossesScore)
    ...
    private fun getBestMove(): Button? {
        // Sprawdź, czy komputer może wygrać w jednym ruchu
        for (button in boardList) {
        ...
        // Sprawdź, czy użytkownik może wygrać w jednym ruchu i zablokuj go
        for (button in boardList) {
            if (button.text.isEmpty()) {
                button.text = NOUGHT
                if (checkForVictory(NOUGHT)) {
